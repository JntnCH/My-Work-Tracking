...
//#region node_modules/.nitro/vite/services/ssr/assets/airtable.functions--LDUbEep.js
...
var syncRecordToAirtable = createServerFn({ method: "POST" }).inputValidator((input) => objectType({ log: workLogSchema }).parse(input)).handler(syncRecordToAirtable_createServerFn_handler, async ({ data }) => {
	...
	const fields = {
		"#รหัส": log.id,
		"#วันที่": log.date,
		"#เวลาเข้า": log.checkInTime,
		"#เวลาออก": log.checkOutTime || "",
	...
	try {
		const url = log.airtableRecordId ? `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(tableName)}/${log.airtableRecordId}` : `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(tableName)}`;
		const method = log.airtableRecordId ? "PATCH" : "POST";
		const res = await fetch(url, {
			...
			body: JSON.stringify(log.airtableRecordId ? { fields } : { records: [{ fields }] })
		});
		if (!res.ok) {
		...
		return {
			success: true,
			recordId: log.airtableRecordId || resJson.id || resJson.records?.[0]?.id,
