    ...
    case "net-income":
      return (
        <div className="clay-card-mint flex h-full min-h-24 flex-col items-center justify-center p-4 text-center sm:p-5">
          ...
          <div
            className="mt-1 w-full text-center text-3xl leading-tight font-black tracking-tight sm:text-4xl"
...
function ChartCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="clay-card flex h-full min-h-0 min-w-0 flex-col items-center justify-center overflow-visible p-5 text-center">
      <h3 className="mb-3 w-full shrink-0 whitespace-nowrap text-center text-sm font-extrabold text-foreground">
        {title}
      </h3>
...
function Stat({
  ...
  const toneCls =
    tone === "success"
      ? "text-[#0C4335] dark:text-[#6EE2BE]"
      : tone === "destructive"
        ? "text-[#C22818] dark:text-[#FFA0B2]"
        : "text-foreground";
  return (
    <div
      className={`clay-card flex h-full min-w-0 flex-col items-center justify-center text-center ${
        compact ? "p-3" : "p-4"
      }`}
    >
      <div className="flex w-full min-w-0 items-center justify-center gap-1.5 text-center text-xs font-bold leading-5 text-muted-foreground">
        {icon}
        <span className="max-w-full whitespace-nowrap">{label}</span>
      </div>
      <div
        className={`mt-1.5 w-full text-center text-xl leading-tight font-black tracking-tight ${toneCls}`}
        data-testid={testId}
      >
        {value}
      </div>
