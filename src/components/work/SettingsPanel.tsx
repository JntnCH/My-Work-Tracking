...
type Props = {
  ...
  onResetThemeSettings: () => Promise<void>;
...
export function SettingsPanel({
  ...
  const [activeTab, setActiveTab] = useState<
    "general" | "worktypes" | "ot" | "theme" | "integrations" | "authentication"
  >("worktypes");
  ...
  useEffect(() => setRateForm(rates), [rates]);
  useEffect(() => setSheetIdInput(spreadsheetId), [spreadsheetId]);
  useEffect(() => setDraftColors(themeSettings), [themeSettings]);
  useEffect(() => {
    setBranchRateForm(branchSettings);
    const selected = branches.find((branch) => branch.id === activeBranchId);
    setBranchNameInput(selected?.name ?? "");
    setBranchCodeInput(selected?.code ?? "");
  ...
  const handleRatesSave = async () => {
    try {
      ...
      const normalizedSheetId = extractSpreadsheetId(sheetIdInput);
      if (normalizedSheetId && normalizedSheetId.length < 10) {
  ...
  const handleResetColors = async () => {
    ...
    toast.info("รีเซ็ตสีของระบบเป็นค่า Google Material เริ่มต้นแล้ว");
  };
  ...
  const selectGooglePreset = (presetId: string) => {
    ...
    setDraftColors(next);
    void onSaveThemeSettings(next);
    toast.success(`เปลี่ยนชุดสีเป็น ${preset.nameEn} แล้ว`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="surface-card flex flex-wrap items-center justify-between gap-3 p-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            จัดการข้อมูลประเภทงาน, OT, สีของธีม และการเชื่อมต่อ Supabase / Airtable ในที่เดียว
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            <Save className="h-4 w-4" /> บันทึกการตั้งค่าทั้งหมด
          </button>
        </div>
      </div>
      ...
      {activeTab === "worktypes" && (
        <div className="surface-card space-y-4 p-5">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs text-muted-foreground">
              ข้อมูลประเภทงานถูกบันทึกลง Supabase เสมอ Refresh หรือเปลี่ยน Browser ข้อมูลไม่หาย (มี
              ID ถาวร, Soft Delete ไม่ทำลายประวัติเก่า)
            </p>
          </div>
        ...
        <div className="surface-card space-y-6 p-5">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border pb-4">
            <div>
              <h3 className="font-bold text-base flex items-center gap-2">
                <Sparkles className="h-5 w-5 text-primary" /> ปรับแต่งธีม &amp; ดีไซน์ (Google-style Theme System)
              </h3>
              <p className="text-xs text-muted-foreground mt-0.5">
                เลือกชุดสี Google Workspace ปรับโหมด Light / Dark ปรับแต่งมุมโค้ง ความหนาแน่น และสี Token ทั้งหมด
              </p>
            </div>
            <button
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-5 gap-1.5">
                {(
                  ...
                  const isSelected =
                    draftColors.borderRadius === id || (!draftColors.borderRadius && id === "normal");
                  return (
                    <button
                      key={id}
                      type="button"
          ...
          <div className="space-y-3 rounded-2xl border border-border bg-muted/30 p-4">
            <div className="flex items-center justify-between gap-2">
              <div>
                <h4 className="text-sm font-bold flex items-center gap-1.5">
                  <Palette className="h-4 w-4 text-primary" /> ตัวอย่างการแสดงผลสด (Live Interactive Preview)
                </h4>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-xl border border-border bg-card p-3 space-y-2">
                <p className="text-[11px] font-semibold text-muted-foreground">Chart Roles (5 สีกราฟ)</p>
          ...
          <div className="space-y-5 pt-2">
            <div className="rounded-xl border border-primary/20 bg-info-soft/40 p-3">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-primary">ปรับแต่งค่าสีเฉพาะตัว (Custom Semantic Tokens)</h4>
                <span className="text-[10px] text-muted-foreground font-mono">11 Tokens + 5 Chart Roles</span>
              </div>
            </div>
      ...
      {activeTab === "integrations" && (
        <div className="surface-card space-y-5 p-5">
          ...
          <SheetsPanel spreadsheetId={spreadsheetId} onChange={onSetSpreadsheetId} />
          ...
          <div className="rounded-xl border border-border bg-card p-4 space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              ข้อมูลประเภทงาน, User Settings และ Work Logs ทั้งหมดถูกจัดเก็บอย่างปลอดภัยบน Supabase
