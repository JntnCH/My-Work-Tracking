    ...
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        className={cn(
          ...
          // Axis tick labels — use muted-foreground so they're visible in both modes
          "[&_.recharts-cartesian-axis-tick_text]:fill-[var(--muted-foreground)]",
          // Cartesian grid lines — override the hardcoded #ccc stroke
          "[&_.recharts-cartesian-grid_line]:stroke-[var(--border)]",
          "[&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-[var(--border)]",
          // Tooltip cursor lines/areas
          "[&_.recharts-curve.recharts-tooltip-cursor]:stroke-[var(--border)]",
          "[&_.recharts-rectangle.recharts-tooltip-cursor]:fill-[var(--muted)]",
          // Remove the hardcoded white stroke on dots/sectors so they use chart colors
          "[&_.recharts-dot[stroke='#fff']]:stroke-transparent",
          "[&_.recharts-sector[stroke='#fff']]:stroke-transparent",
          // Polar / radial elements
          "[&_.recharts-polar-grid_[stroke='#ccc']]:stroke-[var(--border)]",
          "[&_.recharts-radial-bar-background-sector]:fill-[var(--muted)]",
          // Reference lines
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          // Use CSS-variable-based colors so the tooltip is legible in both modes
        ...
        .map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn(
                "flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-[var(--muted-foreground)]",
              )}
            >
              ...
              <span className="text-[var(--foreground)]">{itemConfig?.label}</span>
            </div>
          );
        })}
    </div>
